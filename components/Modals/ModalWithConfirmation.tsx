...
const ModalWithConfirmation: React.FC<Props> = ({isOpen, text, confirm, cancel}: Props) => {
    ...
    return (
        <Modal visible={isOpen} transparent animationType='fade' statusBarTranslucent>
            <CustomView className='items-center justify-center w-full h-full px-5 bg-zinc-900/40 dark:bg-zinc-600/90'>
                <CustomView className='bg-white dark:bg-darkModalBackground' style={styles.modalWrapper}>
                    <CustomText className='text-black dark:text-white' style={styles.modalText}>{text}</CustomText>
                    <CustomView className='flex-row gap-x-2 w-full justify-between'>
                        <CustomTouchableHighlight name='Подтвердить' className="bg-primary items-center justify-center"
                                                  pressFunction={confirm}
